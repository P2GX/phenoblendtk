...
export class UpsetPlotComponent implements OnChanges {
  ...
  private readonly BASE_COLOR = '#8D10CB';      // rgb(0.55, 0.63, 0.80) converted to hex
  private readonly HIGHLIGHT_COLOR = '#66C2A5'; // rgb(0.40, 0.76, 0.65) converted to hex
  ...
  private renderUpsetPlot(): void {
    ...
    // Grid System metrics mirroring your cell, marginal_w, bar_h parameters
    const cellSize = 35;
    ...
    const totalWidth = marginalWidthMax + matrixWidth + margin.left + margin.right;
    const totalHeight = barHeightMax + matrixHeight + margin.top + margin.bottom;
    ...
    // Coordinate Anchors mimicking matplotlib grid specifications
    ...
    barGroup.selectAll('.bar-annotated')
    ...
    barGroup.selectAll('.bar-observed')
    ...
    combinations.forEach((combo, colIdx) => {
      const activeRows = combo.map(gene => yScale(gene)! + yScale.bandwidth() / 2);
    ...
    matrixGroup.selectAll('.tick text')

...
pub fn load_gene_disease_associations<P: AsRef<Path>>(
    fpath: P,
) -> Result<HashMap<TermId, Vec<GeneDiseaseAssociation>>, PhenoblendError> {
    ...
    let mut association_map: HashMap<TermId, Vec<GeneDiseaseAssociation>> = HashMap::new();
